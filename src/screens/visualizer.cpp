...
const NC::FormattedColor &toColor(size_t number, size_t max, bool wrap) {
  ...
  return Config.visualizer_colors[wrap ? index % colors_size
...
Visualizer::Visualizer()
    : Screen(NC::Window(0, MainStartY, COLS, MainHeight, "", NC::Color::Default,
                        NC::Border())),
      m_output_id(-1), m_reset_output(false), m_source_fd(-1),
      m_sample_consumption_rate(5), m_sample_consumption_rate_up_ctr(0),
      ...
      SMOOTH_CHARS_FLIPPED(
          ToWString("▔🮂🮃🮄🬎🮅🮆█")) // https://unicode.org/charts/PDF/U1FB00.pdf
#endif
{
  InitDataSource();
  InitVisualization();
  ...
  m_fftw_input =
      static_cast<double *>(fftw_malloc(sizeof(double) * DFT_TOTAL_SIZE));
  ...
  m_fftw_plan = fftw_plan_dft_r2c_1d(DFT_TOTAL_SIZE, m_fftw_input,
                                     m_fftw_output, FFTW_ESTIMATE);
...
std::wstring Visualizer::title() { return L"Music visualizer"; }
...
void Visualizer::update() {
  ...
  size_t new_samples =
      m_buffered_samples.get(requested_samples, m_rendered_samples);
  if (new_samples == 0)
    return;
  ...
  if (Config.visualizer_in_stereo) {
    ...
    (this->*draw)(m_rendered_samples.data(), m_rendered_samples.size(), 0,
                  w.getHeight());
  }
  w.refresh();
}
...
void Visualizer::DrawSoundWave(const int16_t *buf, ssize_t samples,
  ...
  auto draw_point = [&](size_t x, int32_t y) {
    ...
    w << NC::XY(x, base_y + y) << c << Config.visualizer_chars[0]
...
void Visualizer::DrawSoundWaveStereo(const int16_t *buf_left,
...
// channel.
void Visualizer::DrawSoundWaveFill(const int16_t *buf, ssize_t samples,
                                   size_t y_offset, size_t height) {
  ...
  for (size_t x = 0; x < win_width; ++x) {
    ...
    for (int32_t j = 0; j < point_y; ++j) {
      ...
      w << NC::XY(x, y) << c << Config.visualizer_chars[1]
        << NC::FormattedColor::End<>(c);
    }
  }
}

void Visualizer::DrawSoundWaveFillStereo(const int16_t *buf_left,
                                         const int16_t *buf_right,
                                         ssize_t samples, size_t height) {
...
void Visualizer::DrawSoundEllipse(const int16_t *buf, ssize_t samples, size_t,
                                  size_t height) {
  const size_t half_width = w.getWidth() / 2;
  const size_t half_height = height / 2;

  // Make it so that the loop goes around the ellipse exactly once.
  const double deg_multiplier =
      2 * boost::math::constants::pi<double>() / samples;
  ...
  for (ssize_t i = 0; i < samples; ++i) {
    ...
    w << NC::XY(half_width + x, half_height + y) << c
      << Config.visualizer_chars[0] << NC::FormattedColor::End<>(c);
  }
}
...
void Visualizer::DrawSoundEllipseStereo(const int16_t *buf_left,
                                        const int16_t *buf_right,
                                        ssize_t samples, size_t half_height) {
  ...
  int32_t x, y;
  for (ssize_t i = 0; i < samples; ++i) {
    x = buf_left[i] / 32768.0 *
        (buf_left[i] < 0 ? left_half_width : right_half_width);
    y = buf_right[i] / 32768.0 *
        (buf_right[i] < 0 ? top_half_height : bottom_half_height);
    ...
    w << NC::XY(left_half_width + x, top_half_height + y) << c
      << Config.visualizer_chars[0] << NC::FormattedColor::End<>(c);
  }
}
...
#ifdef HAVE_FFTW3_H
void Visualizer::DrawFrequencySpectrum(const int16_t *buf, ssize_t samples,
                                       size_t y_offset, size_t height) {
  // If right channel is drawn, bars descend from the top to the bottom.
  const bool flipped = y_offset > 0;
  ...
  for (size_t x = 0; x < win_width; ++x) {
    ...
    for (size_t j = 0; j < h; ++j) {
      ...
      w << NC::XY(x, y) << color << ch << NC::FormattedColor::End<>(color);
    }
  }
}

void Visualizer::DrawFrequencySpectrumStereo(const int16_t *buf_left,
                                             const int16_t *buf_right,
                                             ssize_t samples, size_t height) {
          ...
          if (j < bar_height - 1 || idx == size - 1) {
            ...
            if (flipped) {
              if (Config.visualizer_spectrum_smooth_look_legacy_chars)
                ...
                ch = SMOOTH_CHARS[size - idx - 2];
                color =
                    NC::FormattedColor(color.color(), {NC::Format::Reverse});
              }
            } else {
              ch = SMOOTH_CHARS[idx];
            }
...
void Visualizer::DrawSoundGraphStereo(const int16_t *buf_left,
  ...
  for (ssize_t i = 0; i < samples; ++i) {
    ...
    int32_t mixed =
        static_cast<int32_t>(buf_left[i]) + static_cast<int32_t>(buf_right[i]);
    mono[i] = static_cast<int16_t>(mixed / 2);
  }
  DrawSoundGraph(mono.data(), samples, height,
                 w.getHeight() - height); // Draws bottom
}
...
double Visualizer::InterpolateCubic(size_t x, size_t h_idx) {
  ...
  // less than two data points on right, no interp, should never happen unless
  // VERY low DFT size
  return h_next;
}
...
void Visualizer::ApplyWindow(double *output, const int16_t *input,
                             ssize_t samples) {
  ...
  for (unsigned i = 0; i < samples; ++i) {
    double window = a0 - a1 * cos(2 * pi * i / (DFT_NONZERO_SIZE - 1)) +
                    a2 * cos(4 * pi * i / (DFT_NONZERO_SIZE - 1));
    output[i] = window * input[i] / INT16_MAX;
  }
}

double Visualizer::Bin2Hz(size_t bin) { return bin * 44100 / DFT_TOTAL_SIZE; }
...
void Visualizer::GenLogspace() {
  ...
  const size_t win_width = w.getWidth();
  const size_t left_bins = (log10(HZ_MIN) - win_width * log10(HZ_MIN)) /
                           (log10(HZ_MIN) - log10(HZ_MAX));
  // Generate logspaced frequencies
  m_dft_freqspace.resize(win_width);
  const double log_scale =
      log10(HZ_MAX) / (left_bins + m_dft_freqspace.size() - 1);
...
#ifdef HAVE_FFTW3_H
  case VisualizerType::Spectrum:
    Config.visualizer_type = VisualizerType::Graph; // ← cycle to bars/Graph
    break;
  case VisualizerType::Graph:
    Config.visualizer_type = VisualizerType::Ellipse; // ← next after Graph
    break;
#endif
  case VisualizerType::Ellipse:
...
void Visualizer::OpenDataSource() {
  ...
  if (!m_source_port.empty()) {
    ...
    int errcode = getaddrinfo(m_source_location.c_str(), m_source_port.c_str(),
                              &hints, &res);
    if (errcode != 0) {
      Statusbar::printf("Couldn't resolve \"%1%:%2%\": %3%", m_source_location,
                        m_source_port, gai_strerror(errcode));
      return;
    }
    ...
    for (auto addr = res; addr != nullptr; addr = addr->ai_next) {
      ...
      if (m_source_fd >= 0) {
        ...
        if (errcode < 0) {
          std::cerr << "Binding a socket failed: " << strerror(errno)
                    << std::endl;
          CloseDataSource();
        } else
          break;
      } else
        std::cerr << "Creation of socket failed: " << strerror(errno)
                  << std::endl;
    }
...
void Visualizer::FindOutputID() {
  ...
  if (!Config.visualizer_output_name.empty() && m_source_port.empty()) {
    ...
    if (m_output_id == -1)
      Statusbar::printf("There is no output named \"%s\"",
                        Config.visualizer_output_name);
  }
}

void Visualizer::ResetAutoScaleMultiplier() { m_auto_scale_multiplier = 1; }
